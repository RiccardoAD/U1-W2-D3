...
for (let i=0; i<starWarsCharacters.length;i++){
  ...
  const charachOgg= starWarsCharacters[i];
  const name= charachOgg.name;
...
for (let i=0; i<starWarsCharacters.length;i++){
   if(starWarsCharacters[i].gender==="female"){
    femaleCharacters.push(starWarsCharacters[i]); //femaleCharacters.push(starWarsCharacters[i]).name; se volevo solo i nomi delle femmine di star wars 
   }
}
...
/* ESERCIZIO 5
  Utilizza uno switch statement per inserire uno ad uno gli oggetti dei personaggi di "starWarsCharacters" negli array relativi 
  al colore degli occhi precedentemente creati.
